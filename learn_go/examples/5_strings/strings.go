...
func _strings() {
	...
	str := "OLA, TEXTO ESCRITO EM MAIUSCULAS"
	...
	if strings.Contains(lower, "case") {
		fmt.Println("Sim, existe!")
	}

	// strings are arrays of characters
	// printing out characters 3 to 9
	fmt.Println("Catateres 3-9: " + str[3:9])

	// printing out first 5 characters
	fmt.Println("Primeiros cinco: " + str[:5])

	// split a string on a specific character or word
	sentence := "Sou uma frase feita de palavras!"
	...
	fields1 := strings.FieldsFunc("cat,dog:bird", f)
	fmt.Println(fields1)

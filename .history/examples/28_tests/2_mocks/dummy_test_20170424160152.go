...
var _ = Describe("Listener Interactor", func() {
	...
	Describe("when processing HandleMessage method", func() {
		...
		Context("when HandleMessage returns error because of invalid status", func() {
			test := setupListenerMocks()
			msg.State = "xxxx"
			errorMsg1 := "unrecognized state sent in message: [state:xxxx]"

			err := test.interactor.HandleMessage(&msg)

			It("should return an error", func() {
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(Equal(errorMsg1))
			})
		})
	})
})
